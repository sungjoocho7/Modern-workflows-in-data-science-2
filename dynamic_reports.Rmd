...
render_report <- function(path, cntry) {
  rmarkdown::render(
    ...
    output_file = paste0("reports/evs_eport-", cntry, ".html"))}
...
interpret_72 <- function(model) {
  ...
  p_values <- coeffs["sex2", "Pr(>|t|)"]
  estimates <- coeffs["sex2", "Estimate"]
...
interpret_80 <- function(model) {
  ...
  interpretation <- "Based on the model above, it appears that the sex variable is not statistically significant on attitudes national (p>0.05)."
}
  return(interpretation)
}
```
...
```{r echo=FALSE}
cat(interpret_72(reg_v72))
cat(interpret_80(reg_v80))
```
